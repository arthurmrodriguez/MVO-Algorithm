...
function choice = RouletteWheelSelection(weights)
  %Gets the cummulative sum of the weights
  %and generates a random in [0,1] and multiplies it by
  %the end of the cumsum vector
  accumulation = cumsum(weights);
  p = rand() * accumulation(end);
  chosen_index = -1;
  for index = 1 : length(accumulation)
    if (accumulation(index) < p) %For maximization problems use accumulation(index) greater than (>) p
      chosen_index = index;
      break;
    end
  end
